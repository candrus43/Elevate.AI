...
export async function analyzeCallAsync(callId: string, companyId: string, userId: string): Promise<void> {
  try {
    ...
    // 9. Insert analysis
            ...
            computeAndStoreDimensionScores(
              callId, analysisId, companyId, userId, score,
              summary || `${direction} call, ${duration}s`
            ).then(dims => {
              console.log(`Call ${callId} dimension scores: ${dims.map(d => `${d.dimension}=${d.score}`).join(", ")}`);
            }).catch(err => {
              console.error(`Dimension scoring failed for ${callId}:`, err);
            });
  ...
  try {
    ...
    const calls = await db(sql`
      SELECT c.id, c.user_id, c.direction, c.duration_seconds, c.started_at, c.status, c.recording_url,
             ca.overall_score, ca.sentiment, ca.summary, ca.key_topics, ca.objections_detected,
             ca.compliance_issues, ca.talk_ratio_rep, ca.avg_pace_wpm, ca.filler_word_count,
             u.name as rep_name
      FROM calls c
