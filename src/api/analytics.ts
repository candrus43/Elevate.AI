...
export async function handleExecutiveDashboard(req: Request): Promise<Response> {
  try {
    ...
    // ── Business KPIs (require connected CRM — see note) ────────────────────
    // Revenue, conversion, deal, and retention KPIs are gated behind real CRM data.
    // ElevateAI does NOT fabricate business outcomes from conversation scores.
    // Connect a CRM (Salesforce, HubSpot, etc.) to populate these fields.
    const revenueImpact = {
      note: "CRM integration required — business outcome KPIs (revenue, conversion, deals) require a connected CRM or manual data source. These fields are not estimated from call scores.",
      requiresCRM: true,
