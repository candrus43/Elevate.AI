...
/** Full management rights over compliance rules, escalation, and documents. */
export function isComplianceAdmin(user: { role: string } | null): boolean {
  return hasRole(user, ROLES.ADMIN, ROLES.COMPLIANCE_ADMIN);
}

/** Rights to review findings (confirm/dismiss/escalate/resolve). */
export function isComplianceReviewer(user: { role: string } | null): boolean {
  return hasRole(user, ROLES.ADMIN, ROLES.COMPLIANCE_ADMIN, ROLES.COMPLIANCE_REVIEWER, ROLES.MANAGER);
}

// ─── Live Data Access Check (always allowed — live mode is enforced globally) ──