...
// ── KPI Engine ────────────────────────────────────────────────────────────────
import {
  handleListKPIDefinitions, handleGetKPIDefinition, handleCreateKPIDefinition,
  handleUpdateKPIDefinition, handleDeleteKPIDefinition,
  handleListKPIProfiles, handleGetKPIProfile, handleCreateKPIProfile,
  handleUpdateKPIProfile, handleDeleteKPIProfile,
  handleAddProfileItem, handleRemoveProfileItem, handleSetItemOverride,
  handleGetKPITemplates, handleSeedDefaultProfile,
} from "./kpi";
...
export async function routeApi(req: Request): Promise<Response | null> {
  ...
  try {
    ...
    // ── KPI Engine ────────────────────────────────────────────────────────────────
    // Definitions
    if (pathname === "/api/kpi/definitions" && req.method === "GET") return handleListKPIDefinitions(req);
    if (pathname === "/api/kpi/definitions" && req.method === "POST") return handleCreateKPIDefinition(req);
    if (pathname.match(/^\/api\/kpi\/definitions\/[^/]+$/) && req.method === "GET") return handleGetKPIDefinition(req);
    if (pathname.match(/^\/api\/kpi\/definitions\/[^/]+$/) && req.method === "PUT") return handleUpdateKPIDefinition(req);
    if (pathname.match(/^\/api\/kpi\/definitions\/[^/]+$/) && req.method === "DELETE") return handleDeleteKPIDefinition(req);
    // Profiles
    if (pathname === "/api/kpi/profiles" && req.method === "GET") return handleListKPIProfiles(req);
    if (pathname === "/api/kpi/profiles" && req.method === "POST") return handleCreateKPIProfile(req);
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+$/) && req.method === "GET") return handleGetKPIProfile(req);
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+$/) && req.method === "PUT") return handleUpdateKPIProfile(req);
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+$/) && req.method === "DELETE") return handleDeleteKPIProfile(req);
    // Profile items
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+\/items$/) && req.method === "POST") return handleAddProfileItem(req);
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+\/items\/[^/]+$/) && req.method === "DELETE") return handleRemoveProfileItem(req);
    if (pathname.match(/^\/api\/kpi\/profiles\/[^/]+\/items\/[^/]+$/) && req.method === "PUT") return handleSetItemOverride(req);
    // Templates
    if (pathname === "/api/kpi/templates" && req.method === "GET") return handleGetKPITemplates(req);
    if (pathname === "/api/kpi/seed" && req.method === "POST") return handleSeedDefaultProfile(req);
