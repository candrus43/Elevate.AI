...
export async function getOpenAIConfig(companyId: string): Promise<OpenAIConfig | null> {
  const rows = await db(sql`
    SELECT api_key, model, max_tokens, organization_id
...
export async function saveOpenAIConfig(companyId: string, config: Partial<OpenAIConfig>): Promise<void> {
  ...
  if (existing.length > 0) {
    ...
    await db(sql`
      INSERT INTO openai_config (id, company_id, api_key, model, max_tokens, organization_id)
      VALUES (${crypto.randomUUID()}, ${companyId}, ${config.apiKey || ""}, ${config.model || DEFAULT_MODEL}, ${config.maxTokens || MAX_TOKENS}, ${config.organizationId || ""})
    `);
  }
}
...
export async function handleSaveOpenAIConfig(req: Request): Promise<Response> {
  try {
    const user = await getAuthUser(req);
    if (!user) return jsonResponse({ error: "Not authenticated" }, 401);
    if (user.role !== "admin") return jsonResponse({ error: "Only admins can configure OpenAI" }, 403);

    const { apiKey, model, maxTokens, organizationId } = await req.json();
    if (!apiKey) return jsonResponse({ error: "API key is required" }, 400);

    await saveOpenAIConfig(user.companyId, { apiKey, model, maxTokens, organizationId });
