...
import { getEffectiveMode, setIntegrationMode, type IntegrationMode } from "./integration-mode";
...
export const INTEGRATION_PROVIDERS = {
  ...
  observeai: { name: "Observe.ai", category: "ai", docs: "https://docs.observe.ai/api/v1" },
} as const;
...
// ─── PUT /api/integrations/:id/mode ───────────────────────────────────────────
export async function handleSetIntegrationMode(req: Request): Promise<Response> {
  try {
    const user = await getAuthUser(req);
    if (!user) return jsonResponse({ error: "Not authenticated" }, 401);
    if (user.role !== "admin") return jsonResponse({ error: "Only admins can change integration mode" }, 403);

    const integrationId = new URL(req.url).pathname.split("/").slice(-2, -1)[0];
    const { mode } = await req.json();

    if (!mode || !["demo", "live"].includes(mode)) {
      return jsonResponse({ error: "mode must be 'demo' or 'live'" }, 400);
    }
    ...
    await db(sql`
      ...
      WHERE id = ${integrationId} AND company_id = ${user.companyId}
    `);
    ...
    try {
      await db(sql`
        INSERT INTO audit_logs (id, company_id, user_id, action, resource_type, resource_id, details)
        VALUES (${crypto.randomUUID()}, ${user.companyId}, ${user.id}, ${"toggle_integration_mode"}, ${"integration"}, ${integrationId}, ${JSON.stringify({ description: `Changed ${integration[0].provider} mode to ${mode}` })})
      `);
    } catch { /* audit is best-effort */ }

    return jsonResponse({ success: true, mode, message: `${integration[0].provider} mode set to ${mode}` });
  } catch (e) {
    console.error("set integration mode error:", e);
    return jsonResponse({ error: "Failed to set integration mode" }, 500);
  }
}
