 ...
 * Supports demo (simulated) and live (real API) modes.
...
export interface ThoughtSpotConfig {
  apiEndpoint: string;
  apiToken: string;
}

export interface ThoughtSpotQueryResult {
  query: string;
  data: any;
}
...
interface TSAPIResponse<T> {
  success: boolean;
  data?: T;
  error?: string;
}

async function callThoughtSpotAPI<T>(
  config: ThoughtSpotConfig,
  endpoint: string,
  options: { method?: string; body?: any } = {}
): Promise<TSAPIResponse<T>> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { success: false, error: `ThoughtSpot API error (${response.status}): ${errorText}` };
    }
    ...
    if (err.name === "AbortError") {
      return { success: false, error: "ThoughtSpot API request timed out after 30s" };
    }
    return { success: false, error: `ThoughtSpot API request failed: ${err.message}` };
  }
}

/**
 * Execute a TML search query against ThoughtSpot's REST API.
 * Uses the /api/rest/2.0/searchdata endpoint for TML-based queries.
 */
async function queryThoughtSpotLive(
  config: ThoughtSpotConfig,
  queryType: string
): Promise<TSAPIResponse<ThoughtSpotQueryResult>> {
  const queryMappings: Record<string, { tml: string; logicalTable: string }> = {
    sales_performance: {
      tml: 'SELECT rep, deals_closed, pipeline_value, win_rate, avg_deal_size FROM sales_performance ORDER BY deals_closed DESC',
      logicalTable: 'sales_performance',
    },
    pipeline_metrics: {
      tml: 'SELECT stage, count, value FROM pipeline_metrics ORDER BY value DESC',
      logicalTable: 'pipeline_metrics',
    },
    team_metrics: {
      tml: 'SELECT total_reps, avg_win_rate, avg_deal_size, total_pipeline, quota_attainment, avg_call_volume, coaching_hours FROM team_metrics',
      logicalTable: 'team_metrics',
    },
    skill_benchmarks: {
      tml: 'SELECT skill, rep_avg, team_avg, industry_avg, top_performer FROM skill_benchmarks ORDER BY skill',
      logicalTable: 'skill_benchmarks',
    },
  };
  ...
  // ThoughtSpot REST API v2.0 searchdata endpoint
  return callThoughtSpotAPI<ThoughtSpotQueryResult>(config, "/api/rest/2.0/searchdata", {
    method: "POST",
    body: {
      query: { tml: mapping.tml },
      logical_table: mapping.logicalTable,
      format: "json",
    },
  });
}

// ─── SIMULATED THOUGHTSPOT API (Demo Mode) ─────────────────────────────────
...
export async function handleThoughtSpotSync(req: Request): Promise<Response> {
  try {
    ...
    const mode = await getEffectiveMode(user.companyId, user.id, "thoughtspot");
    ...
    for (const queryType of queryTypes) {
      ...
      if (isLive(mode)) {
        // Live mode — call real ThoughtSpot API
        const liveResult = await queryThoughtSpotLive(
          { apiEndpoint: conn.api_endpoint, apiToken: conn.api_token },
          queryType
        );
        result = liveResult;
      } else {
        // Demo mode — simulated data
        result = await simulateThoughtSpotQuery(conn.api_endpoint, conn.api_token, queryType);
      }
