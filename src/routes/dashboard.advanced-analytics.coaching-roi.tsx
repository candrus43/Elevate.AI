...
function CoachingROIAnalytics() {
  ...
  const summary = [
    ...
    { label: "Overall ROI", value: "312%", icon: "📊", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-ink">Coaching ROI Analysis</h1><p className="text-sm text-ink-muted">Detailed return on coaching investments</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl glass-subtle transition-all duration-300">
          <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-ink">Per-Program ROI</h3>
          </div>
          <div className="divide-y divide-edge">
            {programs.map((p, i) => (
              <div key={i} className="px-6 py-3 transition-colors hover:bg-white/[0.02]">
                ...
                <div className="flex items-center gap-3 text-xs text-ink-faint mb-1">
        ...
        <div className="rounded-2xl glass-subtle transition-all duration-300">
          <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-ink">Per-Manager ROI Rankings</h3>
          </div>
          <div className="divide-y divide-edge">
            {managers.map((m, i) => (
              <div key={i} className="px-6 py-3 transition-colors hover:bg-white/[0.02]">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${i === 0 ? "bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-ink-muted"}`}>{i+1}</span>
                    <span className="text-sm font-medium text-ink">{m.name}</span>
                  </div>
                  <span className="text-sm font-bold text-accent-600">{m.roi}%</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-ink-faint ml-8">
                ...
                <div className="ml-8 mt-1 h-1.5 rounded-full bg-white/[0.06]">
                  <div className="h-1.5 rounded-full bg-accent-500" style={{ width: `${Math.min(m.roi / 5, 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-2xl glass-subtle transition-all duration-300">
        <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-ink">Cost Per Improvement Point</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-edge">
                <th className="px-6 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Program</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">Investment</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">Improvement</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">Cost/Point</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">Efficiency</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-edge">
              {programs.map((p, i) => (
                <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-6 py-4 text-sm font-medium text-ink">{p.name}</td>
                  <td className="px-4 py-4 text-right text-sm text-ink-muted">${p.investment.toLocaleString()}</td>
                  <td className="px-4 py-4 text-right text-sm text-green-600 font-semibold">+{p.improvement}%</td>
                  <td className="px-4 py-4 text-right text-sm text-ink-muted">${p.cost.toFixed(2)}</td>
