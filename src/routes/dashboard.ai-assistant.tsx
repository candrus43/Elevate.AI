...
function AIAssistant() {
  ...
  const priorities = [
    { rep: "Taylor Brooks", action: "Review discovery call technique", score: 72, urgency: "high", time: "Today" },
    { rep: "James Kim", action: "Objection handling practice session", score: 65, urgency: "high", time: "Today" },
    { rep: "Casey Nguyen", action: "Closing technique coaching needed", score: 58, urgency: "critical", time: "ASAP" },
  ...
  const strengths = [
    { rep: "Taylor Brooks", strength: "Discovery Questions", score: 92 },
    { rep: "James Kim", strength: "Product Knowledge", score: 88 },
    { rep: "Casey Nguyen", strength: "Objection Handling", score: 54 },
    { rep: "Mike Chen", strength: "Closing", score: 91 },
  ];

  const riskAlerts = [
    { rep: "Casey Nguyen", risk: "Score declined 15% this month", severity: "high", daysSinceImprovement: 21 },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink">AI Manager Assistant</h1>
          <p className="text-sm text-ink-muted mt-1">Daily coaching priorities, insights, and recommendations</p>
        </div>
      ...
      <GlassCard>
        <GlassCardHeader>
          <h3 className="text-lg font-semibold text-ink">Today's Coaching Priorities</h3>
          ...
          {priorities.map((p, i) => (
            <GlassCardRow key={i}>
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-bold text-ink shadow-lg ${
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <a href={`/dashboard/ai-assistant/rep/${p.rep.toLowerCase().replace(" ", "-")}`} className="font-medium text-ink hover:text-accent-300 transition-colors">{p.rep}</a>
                    <GlassBadge color={p.urgency === "critical" ? "red" : p.urgency === "high" ? "amber" : p.urgency === "medium" ? "blue" : "default"}>{p.urgency}</GlassBadge>
                  </div>
                  <p className="text-sm text-ink-muted mt-0.5">{p.action}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 ml-4">
                <span className="text-sm font-medium text-ink">{p.score}%</span>
                <span className="text-xs text-ink-faint w-16 text-right">{p.time}</span>
              </div>
            </GlassCardRow>
          ))}
        </GlassCardBody>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <GlassCard>
          <GlassCardHeader>
            <h3 className="text-lg font-semibold text-ink">Rep Strengths & Weaknesses</h3>
          </GlassCardHeader>
          <GlassCardBody divide>
            {strengths.map((s, i) => (
              <GlassCardRow key={i}>
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-ink">{s.rep}</span>
                    <span className="text-xs text-ink-faint">{s.strength}</span>
                  </div>
        ...
        <GlassCard>
          <GlassCardHeader>
            <h3 className="text-lg font-semibold text-ink">Rep Risk Alerts</h3>
            <GlassBadge color="red">{riskAlerts.length} alerts</GlassBadge>
          </GlassCardHeader>
          <GlassCardBody divide>
            {riskAlerts.length === 0 ? (
              <div className="px-6 py-8 text-center text-sm text-ink-faint">No risk alerts at this time</div>
            ) : (
              riskAlerts.map((r, i) => (
                <GlassCardRow key={i}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-ink">{r.rep}</span>
                        <GlassBadge color={r.severity === "high" ? "red" : "amber"}>{r.severity}</GlassBadge>
                      </div>
                      <p className="text-sm text-ink-muted mt-0.5">{r.risk}</p>
                      <p className="text-xs text-ink-faint mt-1">{r.daysSinceImprovement} days since last improvement</p>
                    </div>
                  </div>
                </GlassCardRow>
              ))
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        {[
          ...
          <a
            key={i}
            href={item.href}
            className="rounded-2xl p-4 transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
            style={{
