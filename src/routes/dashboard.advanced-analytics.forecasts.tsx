...
function Forecasting() {
  ...
  const projections = [
    ...
    { label: "3 Months", value: "84%", improvement: "+10%", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-ink">Forecasting</h1><p className="text-sm text-ink-muted">Projected coaching performance and growth</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {projections.map((p, i) => (
          <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <p className="text-sm text-ink-muted">{p.label} Projection</p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Current Trajectory vs Target</h3>
          <div className="h-48">
          ...
          <div className="flex items-center gap-3 text-xs text-ink-faint mt-2">
            ...
            <span className="flex items-center gap-1"><span className="h-2 w-4 rounded bg-accent-400" /> Projected</span>
        ...
        {/* Scenario Analysis */}
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Scenario Analysis (What-If)</h3>
              ...
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-ink-muted">{s.confidence}% conf</span>
                  </div>
                </div>
                <div className="h-2 rounded-full bg-white/[0.06]">
                  <div className={`h-2 rounded-full ${
                    i === 0 ? "bg-gray-500" : i === 1 ? "bg-blue-500" : i === 2 ? "bg-accent-500" : "bg-emerald-500"
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  <span className="text-[10px] text-ink-muted">Confidence: {s.confidence}%</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-lg font-semibold text-ink mb-4">Forecast Confidence Intervals</h3>
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex items-center gap-4">
              <span className="w-20 text-xs font-medium text-ink-muted">{ci.period}</span>
              <div className="flex-1">
                <div className={`h-3 rounded-full bg-accent-200 dark:bg-accent-800 ${ci.width} relative`}>
                  <div className="absolute left-1/3 top-0 h-3 w-1/3 rounded-full bg-accent-500" />
                  <div className="absolute left-0 top-0 h-full w-0.5 bg-accent-700" />
                  <div className="absolute right-0 top-0 h-full w-0.5 bg-accent-700" />
                </div>
              </div>
              <span className="w-24 text-xs text-right text-ink-muted">{ci.range}</span>
            </div>
          ))}
          <p className="text-[10px] text-ink-muted mt-2">Darker band shows the most likely outcome range. Outer edges show 95% confidence interval.</p>
        </div>
      </div>
    </div>
  );
