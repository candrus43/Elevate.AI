    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-ink-muted">Side-by-side coaching metrics across departments</p>
        </div>
      </div>
        ...
        {departments.map((dept, i) => (
          <div key={i} className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            <div className="flex items-center gap-2 mb-3">
              <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold text-ink ${i === 0 ? "bg-emerald-500" : i === 1 ? "bg-blue-500" : i === 2 ? "bg-accent-500" : i === 3 ? "bg-amber-500" : "bg-gray-500"}`}>{dept.score}</span>
              <span className="text-sm font-medium text-ink">{dept.name}</span>
              <span className={`ml-auto text-xs ${dept.trend === "up" ? "text-green-600" : "text-ink-muted"}`}>{dept.trend === "up" ? "↗" : "→"}</span>
            </div>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <span className="text-ink-faint">Effectiveness</span>
                <span className="font-semibold text-ink">{dept.effectiveness}%</span>
              </div>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-semibold text-ink">{dept.completion}%</span>
              </div>
              <div className="h-1 rounded-full bg-white/[0.06]">
                <div className="h-1 rounded-full bg-blue-500" style={{ width: `${dept.completion}%` }} />
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-ink-faint">Improvement</span>
                <span className="font-semibold text-green-600">+{dept.improvement}%</span>
              </div>
              <p className="text-[10px] text-ink-muted">{dept.reps} active reps</p>
            </div>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-edge">
                <th className="px-6 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Department</th>
                {comparisonMetrics.map(m => (
                  <th key={m} className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">{m}</th>
                ))}
                <th className="px-4 py-3 text-center text-xs font-semibold text-ink-faint uppercase">Rank</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-edge">
              {departments.map((dept, i) => (
                <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-6 py-4 text-sm font-medium text-ink">{dept.name}</td>
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
                      ...
                      i === 1 ? "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-ink-muted" :
                      "bg-gray-100 text-ink-faint dark:bg-gray-800 dark:text-ink-faint"
                    }`}>#{i+1}</span>
                  </td>
                </tr>
              ))}
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-lg font-semibold text-ink mb-4">Metric Comparison Chart</h3>
        <div className="space-y-5">
          {comparisonMetrics.map((metric, i) => (
            <div key={i}>
              <p className="text-sm font-medium text-ink mb-2">{metric}</p>
              <div className="space-y-1.5">
                {departments.map((dept, j) => {
                  ...
                  const color = j === 0 ? "bg-emerald-500" : j === 1 ? "bg-blue-500" : j === 2 ? "bg-accent-500" : j === 3 ? "bg-amber-500" : "bg-gray-500";
                  ...
                  return (
                    <div key={j} className="flex items-center gap-2">
                      <span className="w-28 text-xs text-ink-muted truncate">{dept.name}</span>
                      <div className="flex-1 h-3 rounded-full bg-white/[0.06]">
                        <div className={`h-3 rounded-full ${color}`} style={{ width: `${Math.min(value, 100)}%` }} />
                      </div>
                      <span className="w-12 text-right text-xs font-semibold text-ink">{displayValue}</span>
                    </div>
                  );
                })}
              </div>
