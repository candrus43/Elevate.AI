  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink">SSO / SAML Settings</h1>
          <p className="text-sm text-ink-muted mt-1">Configure single sign-on for your organization</p>
        </div>
      ...
      <GlassCard hover={false}>
        <div className="p-5 sm:p-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-lg font-semibold text-ink">Enable SSO</h3>
              <p className="text-sm text-ink-muted">Allow users to sign in with SAML single sign-on</p>
            </div>
            <label className="relative inline-flex cursor-pointer items-center">
              <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="peer sr-only" />
              <div className="h-6 w-11 rounded-full bg-panel-raised after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all peer-checked:bg-accent-600 peer-checked:after:translate-x-full" />
            </label>
          </div>
          {enabled && (
      ...
      {enabled && (
        <>
          ...
          <GlassCard>
            <GlassCardHeader>
              <h3 className="text-lg font-semibold text-ink">Identity Provider Configuration</h3>
            </GlassCardHeader>
            <div className="p-5 sm:p-6 space-y-6">
              ...
              {configMode === "url" ? (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ink-muted">IdP Metadata URL</label>
                  <div className="flex gap-2">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-ink-muted">Entity ID (SP)</label>
                      <GlassInput type="text" value={entityId} onChange={(e) => setEntityId(e.target.value)} />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-ink-muted">ACS URL</label>
                      <GlassInput type="url" value={acsUrl} readOnly className="!opacity-60" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-ink-muted">IdP SSO URL</label>
                      <GlassInput type="url" value={idpSsoUrl} onChange={(e) => setIdpSsoUrl(e.target.value)} placeholder="https://your-idp.com/sso" />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-ink-muted">IdP Entity ID</label>
                      <GlassInput type="text" value={idpEntityId} onChange={(e) => setIdpEntityId(e.target.value)} placeholder="https://your-idp.com/entity" />
                    </div>
                  </div>
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-ink-muted">X.509 Certificate</label>
                    <textarea
                      rows={4}
                      value={certificate}
                      onChange={(e) => setCertificate(e.target.value)}
                      placeholder="-----BEGIN CERTIFICATE-----&#10;...&#10;-----END CERTIFICATE-----"
                      className="w-full rounded-xl px-3 sm:px-4 py-2.5 text-sm font-mono text-ink placeholder-ink-faint focus:outline-none focus:ring-1 focus:ring-focus/40 transition-all"
                      style={{
          ...
          <GlassCard>
            <GlassCardHeader>
              <h3 className="text-lg font-semibold text-ink">Attribute Mapping</h3>
            </GlassCardHeader>
            <div className="p-5 sm:p-6 space-y-4">
              <p className="text-xs text-ink-faint">Map SAML attributes to ElevateAI user fields</p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ink-muted">Email Attribute</label>
                  <GlassInput type="text" defaultValue="email" />
                  <p className="text-[10px] text-ink-faint mt-0.5">SAML attribute for user email</p>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ink-muted">Name Attribute</label>
                  <GlassInput type="text" defaultValue="displayName" />
                  <p className="text-[10px] text-ink-faint mt-0.5">SAML attribute for user display name</p>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ink-muted">Role Attribute (optional)</label>
                  <GlassInput type="text" defaultValue="" placeholder="role" />
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ink-muted">Default Role</label>
