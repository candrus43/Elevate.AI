...
const PROVIDER_CATALOG: Record<string, { name: string; icon: string; desc: string; color: string; category: string; configFields: { key: string; label: string; type: string; placeholder: string }[] }> = {
  ...
  msteams: { name: "Microsoft Teams", icon: "MT", desc: "Analyze Teams call recordings", color: "bg-accent-600", category: "Video", configFields: [
  ...
  hodu: { name: "Hodu Phone", icon: "HP", desc: "Connect HoduCC phone system — call sync, click-to-dial, live streaming", color: "bg-accent-600", category: "Dialer", configFields: [{ key: "api_key", label: "API Key", type: "password", placeholder: "HoduCC API key" }, { key: "api_url", label: "API URL", type: "text", placeholder: "https://your-hodu-instance.com/api" }] },
  slack: { name: "Slack", icon: "SL", desc: "Get coaching alerts and reports in Slack", color: "bg-accent-500", category: "Communication", configFields: [{ key: "webhook_url", label: "Webhook URL", type: "text", placeholder: "https://hooks.slack.com/services/..." }, { key: "channel", label: "Default Channel", type: "text", placeholder: "#coaching-alerts" }] },
  ...
  thoughtspot: { name: "ThoughtSpot", icon: "TS", desc: "Pull analytics data to enrich coaching dashboards", color: "bg-accent-600", category: "Analytics", configFields: [{ key: "api_key", label: "API Key", type: "password", placeholder: "ThoughtSpot API key" }, { key: "instance_url", label: "Instance URL", type: "text", placeholder: "https://your-instance.thoughtspot.com" }] },
  openai: { name: "AI Provider", icon: "🤖", desc: "AI engine for call analysis, role-play, and coaching — OpenAI-compatible API", color: "bg-green-600", category: "AI", configFields: [{ key: "api_key", label: "API Key", type: "password", placeholder: "sk-... or your provider's API key" }, { key: "base_url", label: "Base URL", type: "text", placeholder: "https://api.openai.com/v1" }, { key: "model", label: "Model", type: "text", placeholder: "gpt-4o-mini, claude-3-sonnet, gemini-pro, etc." }] },
  observeai: { name: "Observe.ai", icon: "OA", desc: "Real conversation intelligence powered by AI", color: "bg-emerald-600", category: "AI", configFields: [
    { key: "api_key", label: "API Key", type: "password", placeholder: "Observe.ai API key" },
    { key: "instance_url", label: "Instance URL", type: "text", placeholder: "https://api.observe.ai/v1" },
    { key: "agent_filter", label: "Agent IDs (comma-separated) — leave empty for all", type: "text", placeholder: "AG-101, AG-102, AG-103" },
  ] },
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-ink-muted mt-1">Connect your tools to ElevateAI</p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${int.color} text-sm font-bold text-ink shadow-lg`}>
                    {int.icon}
                  </div>
                  <div>
                    <h3 className="font-medium text-ink">{int.name}</h3>
                    <span className="text-xs text-ink-faint">{int.category}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${
                    int.status === "connected" ? "bg-green-500/10 text-green-300" :
                    int.status === "disconnected" ? "bg-panel-raised text-ink-muted" :
              ...
              <p className="text-sm text-ink-muted mb-3">{int.desc}</p>

              {/* Action buttons */}
              {!isComingSoon && int.status === "disconnected" && (
                <button onClick={() => openConnectModal(int.provider)}
                  className="inline-block text-sm font-medium text-accent-300 hover:text-accent-300 transition-colors">
                  Connect
                </button>
              )}
      ...
      {connectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-fade-in">
          ...
          >
            ...
            <p className="text-sm text-ink-muted mb-4">Enter your credentials to connect</p>
            <div className="space-y-4">
              {PROVIDER_CATALOG[connectModal.provider]?.configFields.map((field) => (
                <div key={field.key}>
                  <label className="block text-sm font-medium text-ink-muted mb-1">{field.label}</label>
                  <input
                    type={field.type}
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button onClick={() => setConnectModal(null)}
                  className="rounded-xl bg-panel-raised px-4 py-2.5 text-sm font-medium text-ink-muted hover:bg-panel-raised transition-all">
                  Cancel
                </button>
                <button onClick={handleConnect} disabled={connecting}
                  className="rounded-xl bg-gradient-to-r from-accent-600 to-accent-600 px-4 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500 transition-all disabled:opacity-50">
