...
// Business-outcome KPIs (conversion, revenue, deals) require a connected CRM.
// ElevateAI does NOT fabricate business outcomes from conversation scores.
// See Core Principle 3: https://elevate-ai.ctonew.app
const revenueImpact = { note: "CRM integration required", requiresCRM: true, estimatedConversionRate: null as number | null, estimatedDealsFromCoaching: null as number | null };
...
const kpiCards = [
  ...
  { label: "Compliance", value: "96%", change: "2,733/2,847 checks", icon: "🛡️", gradient: "from-accent-500 to-accent-600" },
  { label: "Coaching ROI", value: "+14 pts", change: "CRM required for deals", icon: "📈", gradient: "from-accent-500 to-accent-600" },
];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-ink">Executive Dashboard</h1>
      ...
      <ExecutiveSummaryCard
        ...
        confidence={88}
        summary="Based on 30-day data: 2,847 calls analyzed with avg score of 82%. 7 coaching plans completed with +14pt average improvement. Compliance at 96%. Business-outcome KPIs (conversion, revenue, deals) require a connected CRM — not estimated from call scores. Top improvement areas: objection handling (+22%), talk-to-listen ratio (+15%), and discovery questions (+18%)."
        action={
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <GlassCard padding="none">
          <GlassCardHeader>
            <h3 className="text-lg font-semibold text-ink">Call Trends (Last 7 Days)</h3>
          </GlassCardHeader>
              ...
              {weeklyTrend.map((day, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2">
                    <span className="text-xs text-ink-faint">{day.score}%</span>
                    <div className="w-full flex flex-col items-center" style={{ height: `${Math.max(height, 4)}%` }}>
                      <div className="w-full rounded-t bg-gradient-to-t from-accent-500 to-accent-500 transition-all" style={{ height: "100%" }} />
                    </div>
        ...
        <GlassCard padding="none">
          <GlassCardHeader>
            <div className="flex items-center justify-between w-full">
              ...
              <Link to="/executive/team-health" className="text-sm font-medium text-accent-300 hover:text-accent-300 transition-colors">View all</Link>
            </div>
          </GlassCardHeader>
          <GlassCardBody divide>
            <GlassCardRow>
              <div><p className="text-sm font-medium text-ink">Total Members</p><p className="text-xs text-ink-faint">24 active users</p></div>
              <span className="text-lg font-bold text-ink">24</span>
            </GlassCardRow>
            <GlassCardRow>
              <div><p className="text-sm font-medium text-ink">High Performers</p><p className="text-xs text-ink-faint">Score ≥ 80</p></div>
              <span className="text-lg font-bold text-emerald-400">8</span>
            </GlassCardRow>
            <GlassCardRow>
              <div><p className="text-sm font-medium text-ink">Needs Improvement</p><p className="text-xs text-ink-faint">Score &lt; 60</p></div>
              <span className="text-lg font-bold text-amber-400">3</span>
            </GlassCardRow>
            <GlassCardRow>
              <div><p className="text-sm font-medium text-ink">Inactive</p><p className="text-xs text-ink-faint">No calls this period</p></div>
              <span className="text-lg font-bold text-ink-muted">2</span>
            </GlassCardRow>
          </GlassCardBody>
        </GlassCard>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <RecommendedAction title="Improve Low Performers" description="3 rep(s) scoring below 60. Schedule focused coaching interventions." priority="high" icon="🚨"
            action={<Link to="/executive/team-health" className="text-xs font-medium text-accent-300 hover:text-accent-300">View team details →</Link>} />
          <RecommendedAction title="Scale Coaching Program" description="7 plans completed with +14pt avg improvement. Expand to more reps." priority="medium" icon="📈"
            action={<Link to="/executive/coaching" className="text-xs font-medium text-accent-300 hover:text-accent-300">View ROI analysis →</Link>} />
          <RecommendedAction title="Compliance Check" description="96% compliance rate. On track. Continue monitoring." priority="low" icon="🛡️"
            action={<Link to="/executive/team-health" className="text-xs font-medium text-accent-300 hover:text-accent-300">View details →</Link>} />
        </div>
      </div>
      ...
      <GlassCard padding="none">
        <GlassCardHeader>
          <h3 className="text-lg font-semibold text-ink">Manager Performance</h3>
          <span className="text-xs text-ink-faint">4 managers</span>
        </GlassCardHeader>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.06)" }}>
                <th className="px-6 py-3 text-left text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Manager</th>
                <th className="px-6 py-3 text-left text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Team</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Size</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Score</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Calls</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Plans</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Improvement</th>
                <th className="px-6 py-3 text-center text-[10px] font-semibold text-ink-faint uppercase tracking-wider">Compliance</th>
              </tr>
            </thead>
              ...
              {managers.map((mgr) => (
                <tr key={mgr.managerId} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-accent-500 to-accent-600 text-[10px] font-bold text-white shadow-lg">
                        {mgr.managerName.split(" ").map(n => n[0]).join("").slice(0, 2)}
                      </div>
                      <div>
                        <p className="font-medium text-ink">{mgr.managerName}</p>
                        <p className="text-[10px] text-ink-faint">{mgr.managerEmail}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-3 text-sm text-ink-muted">{mgr.teamName}</td>
                  <td className="px-6 py-3 text-center font-medium text-ink">{mgr.teamSize}</td>
                  ...
                  <td className="px-6 py-3 text-center text-sm text-ink-muted">{mgr.callsAnalyzed}</td>
                  <td className="px-6 py-3 text-center text-sm text-ink-muted">{mgr.coachingPlans}</td>
                  <td className="px-6 py-3 text-center">
                  ...
                  <td className="px-6 py-3 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <span className="text-xs text-ink-muted">{mgr.complianceRate}%</span>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Link to="/executive/forecasting" className="group rounded-2xl p-5 transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-accent-500 to-accent-600 shadow-lg">🔮</div>
            <div><p className="font-medium text-ink group-hover:text-accent-300 transition-colors">Forecasting</p><p className="text-sm text-ink-muted">Revenue projections & trends</p></div>
          </div>
        </Link>
        <Link to="/executive/team-health" className="group rounded-2xl p-5 transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
          style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(139,92,246,0.04) 50%, rgba(255,255,255,0.02) 100%)", backdropFilter: "blur(24px)", border: "1px solid rgba(255, 255, 255, 0.06)" }}>
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-medium text-ink group-hover:text-accent-300 transition-colors">Team Health</p><p className="text-sm text-ink-muted">Detailed team metrics</p></div>
          </div>
        </Link>
        <Link to="/executive/reports" className="group rounded-2xl p-5 transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
          style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(139,92,246,0.04) 50%, rgba(255,255,255,0.02) 100%)", backdropFilter: "blur(24px)", border: "1px solid rgba(255, 255, 255, 0.06)" }}>
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-medium text-ink group-hover:text-accent-300 transition-colors">Reports</p><p className="text-sm text-ink-muted">Scheduled reports & exports</p></div>
          </div>
        </Link>
      </div>
    </div>
