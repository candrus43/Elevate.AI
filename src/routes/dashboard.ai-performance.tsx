...
function AIPerformance() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-ink">AI Performance Reviews</h1>
        <p className="text-sm text-ink-faint">AI-generated performance reviews and insights</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-ink">{stat.value}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
        <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-ink">Recent Reviews</h3>
        </div>
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          {[
            { name: "Taylor Brooks", role: "SDR", period: "Q2 2026", score: 85, trend: "up", summary: "Strong improvement in discovery calls. Continue working on closing techniques." },
            ...
            { name: "Casey Nguyen", role: "SDR", period: "Q2 2026", score: 61, trend: "down", summary: "Struggling with talk-to-listen ratio. Recommend additional coaching." },
            ...
            <div key={i} className="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="font-medium text-gray-900 dark:text-ink">{review.name}</p>
                  <p className="text-xs text-ink-faint">{review.role} · {review.period}</p>
                </div>
              ...
              <p className="text-sm text-ink-faint">{review.summary}</p>
              <div className="mt-2 flex items-center gap-2">
                <span className="rounded-full bg-accent-100 px-2 py-0.5 text-xs font-medium text-accent-700 dark:bg-accent-900/50 dark:text-accent-300">AI Generated</span>
                <span className="text-xs text-ink-muted">92% confidence</span>
              </div>
            </div>
          ))}
        </div>
