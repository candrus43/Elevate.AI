...
function MyProgress() {
  ...
  const overall = [
    { label: "Courses Completed", value: "3 / 6", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-ink">My Progress</h1><p className="text-sm text-ink-muted">Track your learning journey</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div className="flex items-center justify-between mb-1">
                  <div>
                    ...
                    <span className={`ml-2 text-[10px] font-medium ${
                      c.status === "Completed" ? "text-green-600" : c.status === "In Progress" ? "text-amber-600" : "text-ink-muted"
                    }`}>{c.status}</span>
                  </div>
                  <span className={`text-sm font-bold ${c.progress === 100 ? "text-green-600" : c.progress > 0 ? "text-amber-600" : "text-ink-muted"}`}>{c.progress}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-white/[0.06]">
                  <div className={`h-1.5 rounded-full ${c.progress === 100 ? "bg-green-500" : "bg-accent-500"}`} style={{ width: `${c.progress}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-sm font-semibold text-ink mb-4">Quiz Score History</h3>
          <div className="h-40">
          ...
          <div className="flex items-center justify-between text-xs text-ink-faint mt-2">
            <span>Avg: <strong className="text-ink">{Math.round(quizScores.reduce((a,b) => a+b,0)/quizScores.length)}%</strong></span>
