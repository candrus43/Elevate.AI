...
export const Route = createFileRoute("/executive/coaching/skill-heatmaps")({
...
function SkillHeatmaps() {
  ...
  const getColor = (score: number) => {
    if (score >= 85) return "bg-emerald-500 text-ink";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/executive/coaching" className="text-2xl text-ink-muted hover:text-gray-600 dark:hover:text-ink-muted">&larr;</Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-ink">Skill Heatmaps</h1>
          <p className="text-sm text-ink-muted">Visualize skill strengths and gaps across teams</p>
        </div>
      ...
      <div className="flex items-center gap-4 rounded-xl border border-gray-200 bg-white px-6 py-3 dark:border-gray-700 dark:bg-gray-900">
        ...
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-emerald-500" /><span className="text-xs text-ink-faint">Strong (85%+)</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-emerald-300" /><span className="text-xs text-ink-faint">Good (75-84%)</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-amber-300" /><span className="text-xs text-ink-faint">Needs Work (65-74%)</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-red-400" /><span className="text-xs text-ink-faint">Weak (&lt;65%)</span></span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-edge">
              <th className="px-4 py-3 text-left text-xs font-semibold text-ink-muted uppercase tracking-wider w-48">Team</th>
              {skills.map(s => (
                <th key={s} className="px-3 py-3 text-center text-xs font-semibold text-ink-muted uppercase tracking-wider">{s}</th>
              ))}
            ...
            {teams.map((team, i) => {
              ...
              return (
                <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-4 py-3 text-sm font-medium text-ink">{team}</td>
                  {skills.map((skill, j) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setSelectedCell(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-ink">Skill Detail</h3>
              <button onClick={() => setSelectedCell(null)} className="text-2xl text-ink-muted hover:text-gray-600">&times;</button>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-ink-faint">Team</span>
                <span className="text-sm font-medium text-ink">{selectedCell.team}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-ink-faint">Skill</span>
                <span className="text-sm font-medium text-ink">{selectedCell.skill}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-ink-faint">Score</span>
              ...
              <button
                ...
                className="w-full rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 py-2 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500 mt-2"
              >
                Schedule Coaching
              </button>
            </div>
