  ...
  if (submitted) {
    ...
    return (
      <div className="space-y-6">
        <Link to="/dashboard/academy/quizzes" className="inline-flex items-center gap-2 text-sm text-accent-600 hover:text-accent-500">&larr; Back to Quizzes</Link>
        <div className="rounded-xl border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <h2 className="text-2xl font-bold text-ink mb-2">{passed ? "Congratulations!" : "Keep Practicing!"}</h2>
          <p className="text-ink-faint mb-4">You scored <span className={`text-3xl font-bold ${passed ? "text-green-600" : "text-red-600"}`}>{score}%</span></p>
          <p className="text-sm text-ink-faint mb-6">{passed ? "You passed the quiz!" : "You need 80% to pass. Try again!"}</p>
          ...
          <div className="flex items-center justify-center gap-3">
            <Link to={`/dashboard/academy/quiz/${quizId}`} className="rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 px-6 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500">Try Again</Link>
            <Link to="/dashboard/academy/quizzes" className="rounded-xl border border-gray-200 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted">More Quizzes</Link>
          </div>
        </div>
        {/* Answer Review */}
        <div className="rounded-2xl glass-subtle transition-all duration-300">
          <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700"><h3 className="text-lg font-semibold text-ink">Answer Review</h3></div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/dashboard/academy/quizzes" className="text-2xl text-ink-muted hover:text-gray-600 dark:hover:text-ink-muted">&larr;</Link>
        <div className="flex-1"><h1 className="text-2xl font-bold text-ink">Enterprise Sales Fundamentals</h1><p className="text-sm text-ink-muted">Answer each question carefully</p></div>
      </div>

      {/* Progress */}
      <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-ink-faint">Question {currentQuestion + 1} of {totalQuestions}</span>
          <span className="text-sm font-medium text-ink-muted">{progress}%</span>
        </div>
        <div className="h-2 rounded-full bg-white/[0.06]">
          <div className="h-2 rounded-full bg-gradient-to-r from-accent-500 to-accent-500 transition-all duration-300" style={{ width: `${progress + (isAnswered ? 100 / totalQuestions : 0)}%` }} />
        </div>
      </div>

      {/* Question */}
      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <p className="text-lg font-medium text-ink mb-6">{questions[currentQuestion].q}</p>
        <div className="space-y-3">
          {questions[currentQuestion].options.map((opt, i) => (
            <button key={i} onClick={() => handleAnswer(i)} className={`w-full rounded-xl border p-4 text-left text-sm transition-all ${answers[currentQuestion] === i ? "border-accent-300 bg-accent-50 dark:border-accent-600 dark:bg-accent-900/20" : "border-gray-200 bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:hover:bg-gray-800"}`}>
              <span className="font-medium text-ink">{String.fromCharCode(65 + i)}. {opt}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <button onClick={handlePrev} disabled={currentQuestion === 0} className="rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted">&larr; Previous</button>
        {currentQuestion === totalQuestions - 1 ? (
          <button onClick={handleSubmit} className="rounded-xl bg-green-600 px-6 py-2.5 text-sm font-medium text-ink hover:bg-green-500">Submit Quiz</button>
        ) : (
          <button onClick={handleNext} disabled={!isAnswered} className="rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 px-5 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500 disabled:opacity-50">Next &rarr;</button>
        )}
      </div>
    </div>
  );
