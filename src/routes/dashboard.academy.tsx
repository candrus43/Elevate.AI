...
function AcademyDashboard() {
  ...
  const stats = [
    ...
    { label: "Quiz Avg Score", value: "82%", color: "from-accent-500 to-accent-600", icon: "📝" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-ink">Coaching Academy</h1>
        <p className="text-sm text-ink-muted mt-1">Enterprise training platform — learn, practice, and certify</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 sm:grid-cols-6">
        {quickLinks.map((link, i) => (
          <Link
            key={i}
            to={link.href}
            className="rounded-2xl p-3 sm:p-4 text-center transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
            style={{
          ...
          >
            ...
            <p className="text-[11px] font-medium text-ink-muted">{link.label}</p>
          </Link>
        ))}
      </div>
          ...
          <GlassCardBody divide>
            {inProgress.length === 0 ? (
              <div className="px-5 sm:px-6 py-8">
                <p className="text-sm text-ink-muted text-center">No courses in progress. Start learning today!</p>
              </div>
            ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-2 rounded-full" style={{ background: "rgba(255, 255, 255, 0.06)" }}>
                        <div className="h-2 rounded-full bg-gradient-to-r from-accent-500 to-accent-500" style={{ width: `${item.progress}%` }} />
                      </div>
                      <span className="text-xs font-medium text-ink-muted">{item.progress}%</span>
                    </div>
                    <p className="text-[10px] text-ink-faint mt-1">Due in {item.due}</p>
                  </div>
                </GlassCardRow>
              ))
            )}
        ...
        <GlassCard>
          ...
          <GlassCardBody divide>
            {recommended.length === 0 ? (
              <div className="px-5 sm:px-6 py-8">
                <p className="text-sm text-ink-muted text-center">No recommendations yet. Check back soon!</p>
              </div>
            ) : (
                ...
                <GlassCardRow key={i}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[10px] text-ink-faint">{item.duration}</span>
