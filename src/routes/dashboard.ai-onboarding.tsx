  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-ink">AI-Guided Onboarding</h1>
          <p className="text-sm text-ink-faint">Let AI guide you through setting up ElevateAI</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-ink mb-4">Setup Progress</h2>
              ...
              {steps.map((s, i) => (
                <div key={i} className="relative flex items-start gap-4 pb-6 last:pb-0">
                  ...
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-bold ${
                    s.status === "completed" ? "bg-green-500 text-ink" : "bg-gray-200 text-ink-faint dark:bg-gray-700 dark:text-ink-muted"
                  }`}>
                    {s.status === "completed" ? "✓" : s.num}
                  </div>
                  <div className="flex-1 pt-1">
                    <div className="flex items-center justify-between">
                      <h3 className="font-medium text-gray-900 dark:text-ink">{s.title}</h3>
                      {s.status === "completed" ? (
                        <span className="text-xs text-green-600">Complete</span>
                      ) : (
                        <button onClick={() => setStep(s.num)} className="rounded-lg bg-accent-600 px-3 py-1 text-xs font-medium text-ink hover:bg-accent-500">Start</button>
                      )}
                    </div>
                    <p className="text-sm text-ink-faint mt-0.5">{s.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-ink mb-3">AI Insights</h3>
            <div className="space-y-3">
              <div className="rounded-lg bg-accent-50 p-3 dark:bg-accent-900/20">
                <div className="flex items-center gap-1 text-xs text-accent-700 dark:text-accent-300 mb-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-accent-500 animate-pulse" />
                  AI Tip
                </div>
                <p className="text-xs text-accent-600 dark:text-accent-400">Based on your industry, we recommend starting with compliance rules first.</p>
              </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-ink mb-3">Estimated Time</h3>
            <p className="text-2xl font-bold text-gray-900 dark:text-ink">~15 min</p>
            <p className="text-xs text-ink-faint mt-1">Complete all steps to go live</p>
          </div>
        </div>
      </div>
    </div>
