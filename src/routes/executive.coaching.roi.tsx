...
export const Route = createFileRoute("/executive/coaching/roi")({
...
function CoachingROI() {
  ...
  const roiSummary = [
    ...
    { label: "Overall ROI Ratio", value: "312%", icon: "📊", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ink">Coaching ROI Analysis</h1>
          <p className="text-sm text-ink-muted">Measure the return on coaching investments</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Score Improvement vs Time Invested</h3>
          <div className="h-56">
          ...
          <p className="text-xs text-ink-faint mt-2">Bubble size indicates total investment amount</p>
        </div>

        {/* Cost Per Improvement Point */}
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Cost Per Point</h3>
            ...
            {programs.sort((a, b) => a.costPerPoint - b.costPerPoint).slice(0, 5).map((p, i) => (
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-bold text-ink">${p.costPerPoint.toFixed(2)}</span>
                </div>
                <div className="h-2 rounded-full bg-white/[0.06]">
                  <div className="h-2 rounded-full bg-gradient-to-r from-green-500 to-accent-500" style={{ width: `${(1 - (p.costPerPoint - 1.5) / 4) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-2xl glass-subtle transition-all duration-300">
        <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-ink">Per-Program ROI Comparison</h3>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-edge">
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-ink-faint uppercase">ROI</th>
            ...
            <tbody className="divide-y divide-edge">
              {programs.sort((a, b) => b.roi - a.roi).map((p, i) => (
                <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-4 text-right text-sm text-ink-muted">${p.costPerPoint.toFixed(2)}</td>
                  <td className="px-4 py-4 text-right text-sm">
