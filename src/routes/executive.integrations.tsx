...
const PROVIDER_CATALOG: Record<string, { name: string; icon: string; desc: string; color: string; category: string }> = {
  ...
  hodu: { name: "Hodu Phone", icon: "HP", desc: "Connect HoduCC phone system for call sync, click-to-dial, and live streaming", color: "bg-accent-600", category: "Dialer" },
  five9: { name: "Five9", icon: "F9", desc: "Integrate with Five9 contact center", color: "bg-green-600", category: "Dialer" },
  slack: { name: "Slack", icon: "SL", desc: "Get coaching alerts and reports in Slack", color: "bg-accent-500", category: "Communication" },
  ...
  thoughtspot: { name: "ThoughtSpot", icon: "TS", desc: "Pull analytics data to enrich coaching dashboards", color: "bg-accent-600", category: "Analytics" },
  msteams: { name: "Microsoft Teams", icon: "MT", desc: "Analyze Teams call recordings", color: "bg-accent-600", category: "Video" },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-ink-muted">Connect your tools to ElevateAI for seamless data sync and automation</p>
      </div>
          ...
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
              filter === cat
                ...
                : "text-ink-faint hover:text-ink-muted bg-panel-raised"
            }`}
          >
            {cat === "all" ? "All" : cat}
          </button>
            ...
            <div className="flex items-start gap-4">
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${integration.color} shadow-lg shrink-0`}>
                {integration.icon.length > 2 ? (
                  ...
                  <span className="text-xs font-bold text-ink">{integration.icon}</span>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <h3 className="font-medium text-ink group-hover:text-accent-300 transition-colors truncate">{integration.name}</h3>
                  <GlassBadge variant={integration.status === "connected" ? "success" : "default"}>
                    {integration.status === "connected" ? "Connected" : "Disconnected"}
                  </GlassBadge>
                </div>
                <p className="text-sm text-ink-muted line-clamp-2">{integration.desc}</p>
                <span className="inline-block mt-2 text-[10px] text-ink-faint uppercase tracking-wider">{integration.category}</span>
              </div>
            </div>
          </Link>
        ))}
