  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate({ to: "/dashboard/integrations" })} className="text-2xl text-ink-muted hover:text-gray-600 dark:hover:text-ink-muted">&larr;</button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-ink">Webhooks</h1>
          <p className="text-sm text-ink-faint">Manage webhook endpoints for event-driven integrations</p>
        </div>
      </div>

      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-ink">Your Webhooks</h3>
          <button className="rounded-xl bg-accent-600 px-4 py-2 text-sm font-medium text-ink hover:bg-accent-500">+ Add Endpoint</button>
        </div>
            ...
            <div key={i} className="flex items-start justify-between rounded-lg border border-gray-200 p-4 dark:border-gray-700">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <code className="text-sm text-gray-900 dark:text-ink truncate block">{hook.url}</code>
                </div>
                <p className="text-xs text-ink-faint mt-1">Events: {hook.events}</p>
                <p className="text-xs text-ink-muted mt-0.5">Last delivery: {hook.lastDelivery}</p>
              </div>
