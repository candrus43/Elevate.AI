...
export const Route = createFileRoute("/executive/coaching/summaries")({
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-ink">Executive Summaries</h1>
          <p className="text-sm text-ink-muted">AI-generated coaching effectiveness briefs</p>
        </div>
        <select value={period} onChange={(e) => setPeriod(e.target.value)} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-focus/40">
      ...
      {/* Summary Narrative */}
      <div className="rounded-xl border border-gray-200 bg-gradient-to-br from-accent-500/[0.04] to-accent-500/[0.04] p-6 dark:border-gray-700">
        <div className="flex items-center gap-2 mb-4">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-100 text-sm dark:bg-accent-900/50">🤖</span>
          <h3 className="text-lg font-semibold text-ink">AI-Generated Summary</h3>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {[
          ...
          <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            ...
            <p className="text-xs text-ink-faint mb-0.5">{item.label}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-lg font-semibold text-ink mb-4">Key Insights &amp; Trends</h3>
          ...
          {[
            ...
            <div key={i} className="flex items-start gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
              ...
              <div>
                ...
                <p className="text-xs text-ink-faint">{insight.desc}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={i} className="px-6 py-4 transition-colors hover:bg-white/[0.02]">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-ink-faint">{rec.reason}</p>
                  <p className="text-xs text-accent-600 dark:text-accent-400 mt-1">Expected impact: {rec.impact}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="flex justify-end gap-3">
        <button className="rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted dark:hover:bg-gray-800">Export as PDF</button>
        <button className="rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted dark:hover:bg-gray-800">Save Summary</button>
        <button className="rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 px-5 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500">Share Report</button>
      </div>
    </div>
  );
}