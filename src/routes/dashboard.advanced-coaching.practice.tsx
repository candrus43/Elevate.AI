...
function PracticeAssignments() {
  ...
  const assignments = [
    {
      id: "a1", rep: "Taylor Brooks", title: "Objection Handling: Price Pushback", type: "roleplay", difficulty: "Intermediate",
    ...
    {
      id: "a3", rep: "Casey Nguyen", title: "Closing: Trial Close Techniques", type: "scenario", difficulty: "Advanced",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-ink">Practice Assignments</h1>
          <p className="text-sm text-ink-faint dark:text-ink-muted">AI-generated practice tasks tailored to skill gaps</p>
        </div>
      </div>
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold text-ink ${
                    a.type === "roleplay" ? "bg-accent-500" : a.type === "scenario" ? "bg-accent-500" : a.type === "review" ? "bg-emerald-500" : "bg-amber-500"
                  }`}>
                    {a.type === "roleplay" ? "🎭" : a.type === "scenario" ? "📋" : a.type === "review" ? "📖" : "✍️"}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-gray-900 dark:text-ink">{a.rep}</p>
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${
                        ...
                        "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-ink-muted"
                      }`}>{a.status.replace("-", " ")}</span>
                    </div>
                    <p className="text-sm text-gray-900 dark:text-ink">{a.title}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-ink-faint">Due {a.dueDate}</p>
                </div>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${
                    a.difficulty === "Advanced" ? "bg-accent-100 text-accent-700 dark:bg-accent-900/50 dark:text-accent-300" :
                    a.difficulty === "Intermediate" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300" :
                    "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300"
                  }`}>{a.difficulty}</span>
                  <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-ink-muted capitalize">{a.type}</span>
                  {a.skills.map((s, i) => (
                    <span key={i} className="text-xs text-ink-faint hidden sm:inline">{s}{i < a.skills.length - 1 ? "," : ""}</span>
                  ))}
                </div>
                {a.status !== "completed" && (
                  <button className="rounded-lg bg-accent-600 px-3 py-1.5 text-xs font-medium text-ink hover:bg-accent-500">Start</button>
                )}
              </div>
            </div>
          </div>
