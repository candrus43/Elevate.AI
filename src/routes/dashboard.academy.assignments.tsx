...
function ManagerAssignments() {
  ...
  const statusColors = {
    "Pending": "transition-colors hover:bg-panel-raised text-ink-muted",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1"><h1 className="text-2xl font-bold text-ink">Manager Assignments</h1><p className="text-sm text-ink-muted">Assign training content to your team</p></div>
        <button onClick={() => setShowAssign(true)} className="rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 px-4 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500">+ Assign</button>
      </div>

      {/* Filters */}
      <div className="flex items-center gap-3">
        <label className="text-sm text-ink-faint">Filter by rep:</label>
        <select value={selectedRep} onChange={(e) => setSelectedRep(e.target.value)} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-focus/40">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-edge">
                <th className="px-6 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Rep</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Item</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Type</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Due</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-edge">
              {filtered.map((a, i) => (
                <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                  <td className="px-6 py-3.5 text-sm font-medium text-ink">{a.rep}</td>
                  <td className="px-4 py-3.5 text-sm text-ink-muted">{a.item}</td>
                  <td className="px-4 py-3.5">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      a.type === "Course" ? "bg-accent-100 text-accent-700 dark:bg-accent-900/50 dark:text-accent-300" :
                  ...
                  <td className="px-4 py-3.5 text-sm text-ink-muted">{a.due}</td>
                  <td className="px-4 py-3.5">
      ...
      {showAssign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setShowAssign(false)}>
          <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold text-ink">Assign Content</h3><button onClick={() => setShowAssign(false)} className="text-2xl text-ink-muted hover:text-gray-600">&times;</button></div>
            <div className="space-y-4">
              <div><label className="mb-1.5 block text-sm font-medium text-ink-muted">Rep</label>
              ...
              <div><label className="mb-1.5 block text-sm font-medium text-ink-muted">Content Type</label>
                <select className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-ink">
                  <option>Course</option><option>Quiz</option><option>Content</option><option>Certification</option>
                </select>
              </div>
              <div><label className="mb-1.5 block text-sm font-medium text-ink-muted">Item</label>
                <select className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-ink">
              ...
              <button className="w-full rounded-xl text-white bg-gradient-to-r from-accent-600 to-accent-600 py-2.5 text-sm font-medium text-white hover:from-accent-500 hover:to-accent-500">Assign</button>
            </div>
          </div>
        </div>
      )}
