    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-ink-faint">5 reports</span>
        </div>
      </div>
          ...
          return (
            <GlassCard key={report.id} padding="md" hover glow>
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-medium text-ink">{report.name}</h3>
                <GlassBadge color={isActive ? "green" : "default"}>
                  {isActive ? "Active" : "Paused"}
                </GlassBadge>
              </div>
              <div className="space-y-1.5 text-sm">
                <p className="text-ink-faint">📅 {scheduleLabel}</p>
                <p className="text-ink-faint">📄 {report.report_type}</p>
                <p className="text-ink-faint">👥 {report.recipients}</p>
                {report.last_sent_at && (
                  <p className="text-ink-faint">🕐 Last sent: {new Date(report.last_sent_at).toLocaleDateString()}</p>
                )}
              </div>
