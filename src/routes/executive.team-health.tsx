    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-ink-muted">Detailed team performance metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <GlassStatCard label="Coaching Completion" value="7" change="5 active plans" color="from-accent-500 to-accent-600" icon={<span>🎯</span>} />
      </div>
        ...
        {teams.map((team, i) => (
          <GlassCard key={i} padding="md">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mt-4">
              <div><p className="text-xs text-ink-faint">Reps</p><p className="font-medium text-ink">{team.reps}</p></div>
              <div><p className="text-xs text-ink-faint">Calls</p><p className="font-medium text-ink">{team.calls}</p></div>
              <div><p className="text-xs text-ink-faint">Avg Score</p><p className="font-medium text-ink">{team.avgScore}%</p></div>
              <div><p className="text-xs text-ink-faint">Coaching</p><p className="font-medium text-ink">{team.coaching}%</p></div>
            </div>
          </GlassCard>
        ))}
      </div>
      ...
      <GlassCard padding="none">
        <GlassCardHeader>
          <h3 className="text-lg font-semibold text-ink">Top Objections</h3>
        </GlassCardHeader>
        <div className="p-5 sm:p-6 space-y-2">
          {topObjections.map((obj, i) => (
            <div key={i} className="flex items-center gap-3">
              <span className="w-6 text-xs text-ink-faint">{i + 1}.</span>
              <span className="flex-1 text-sm text-ink capitalize">{obj.objection.replace(/_/g, " ")}</span>
              ...
              <span className="w-8 text-right text-xs text-ink-faint">{obj.count}</span>
            </div>
          ))}
        </div>
      </GlassCard>
      ...
      <GlassCard padding="none">
        <GlassCardHeader>
          <h3 className="text-lg font-semibold text-ink">Sentiment Distribution</h3>
        </GlassCardHeader>
          ...
          <div className="flex items-end gap-4 h-32">
            {sentimentData.map((s) => (
              <div key={s.sentiment} className="flex-1 flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-ink-faint capitalize">{s.sentiment}</span>
              </div>
            ))}
          </div>
          <div className="mt-6 flex justify-center gap-6 text-sm">
            <div className="text-center">
              <p className="text-xs text-ink-faint">Total Calls</p>
              <p className="text-lg font-bold text-ink">2,847</p>
            </div>
            <div className="h-8 w-px" style={{ background: "rgba(255,255,255,0.08)" }} />
            <div className="text-center">
              <p className="text-xs text-ink-faint">Positive Rate</p>
              <p className="text-lg font-bold text-emerald-400">54%</p>
            </div>
            <div className="h-8 w-px" style={{ background: "rgba(255,255,255,0.08)" }} />
            <div className="text-center">
              <p className="text-xs text-ink-faint">Negative Rate</p>
