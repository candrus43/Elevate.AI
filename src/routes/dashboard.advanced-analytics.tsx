...
function AdvancedAnalytics() {
  ...
  const kpis = [
    ...
    { label: "Coaching ROI", value: "312%", change: "+48%", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-ink">Advanced Coaching Analytics</h1>
        <p className="text-sm text-ink-muted mt-1">Deep insights into coaching performance and impact</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-2">
          {quickLinks.map((link, i) => (
            <Link
              key={i}
              to={link.href}
              className="flex items-center gap-3 rounded-2xl p-3.5 transition-all duration-300 hover:border-accent-500/25 hover:translate-y-[-1px]"
              style={{
            ...
            >
              ...
              <span className="text-sm font-medium text-ink-muted">{link.label}</span>
            </Link>
          ))}
        </div>
        ...
        <GlassCard className="lg:col-span-2">
          <GlassCardHeader>
            <h3 className="text-lg font-semibold text-ink">Rep Improvement Timeline</h3>
          </GlassCardHeader>
          <GlassCardBody>
