  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1"><h1 className="text-2xl font-bold text-ink">Alerts & Notifications</h1><p className="text-sm text-ink-muted">Stay informed about coaching activities</p></div>
        <div className="flex gap-2">
          <button className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted">Mark All Read</button>
          <button className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-ink-muted">Dismiss All</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-all ${filter === f.id ? "text-white bg-gradient-to-r from-accent-600 to-accent-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-white-muted dark:hover:bg-gray-700"}`}>
            {f.label} ({f.count})
          </button>
        ))}
      </div>
      ...
      <div className="rounded-2xl glass-subtle transition-all duration-300">
        <div className="divide-y divide-edge">
          {filtered.map((alert) => (
            <div key={alert.id} className={`flex items-start gap-4 px-6 py-4 transition-colors hover:bg-white/[0.02] ${!alert.read ? "bg-accent-50/50 dark:bg-accent-900/10" : ""}`}>
              <div className="flex flex-col items-center gap-1.5 pt-1">
                <span className={`h-2.5 w-2.5 rounded-full ${severityDots[alert.severity]}`} />
                {!alert.read && <span className="h-1.5 w-1.5 rounded-full bg-accent-500" />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs text-ink-muted">
                  <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-600 dark:bg-gray-800 dark:text-ink-muted">{alert.type}</span>
                  <span>{alert.time}</span>
                </div>
              </div>
              <button className="shrink-0 rounded-lg px-2 py-1 text-xs text-ink-faint transition-colors hover:bg-white/[0.02]">
                {alert.read ? "Mark Unread" : "Mark Read"}
              </button>
            </div>
          ))}
          {filtered.length === 0 && <div className="px-6 py-8 text-center text-sm text-ink-faint">No alerts found.</div>}
        </div>
      </div>
    </div>
  );
