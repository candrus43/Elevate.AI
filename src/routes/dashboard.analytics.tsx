    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink-muted">Team performance insights</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard label="Avg Score" value={avgScore} hint={`${filteredCalls.length} calls analyzed`} />
        <MetricCard label="High Performers" value={String(highScoreCount)} hint="Score ≥ 85" tone="positive" />
        <MetricCard label="Needs Improvement" value={String(lowScoreCount)} hint="Score < 70" tone="warning" />
        <MetricCard label="Active Reps" value={String(team.length)} hint={filteredCalls.length > 0 ? `${(filteredCalls.length / Math.max(team.length, 1)).toFixed(0)} calls/rep` : "No data"} />
      </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-ink-muted">High (85+)</span>
              </div>
              <span className="text-xs font-mono text-ink-muted">{highScoreCount}</span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-ink-muted">Medium (70-84)</span>
              </div>
              <span className="text-xs font-mono text-ink-muted">{mediumScoreCount}</span>
            </div>
            <div className="h-3 w-full rounded-full bg-panel-raised overflow-hidden">
              <div
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-ink-muted">Low (&lt;70)</span>
              </div>
              <span className="text-xs font-mono text-ink-muted">{lowScoreCount}</span>
            </div>
            <div className="h-3 w-full rounded-full bg-panel-raised overflow-hidden">
              <div
      ...
      <ChartCard title="Rep Performance">
        {repStats.length === 0 ? (
          <div className="py-6 text-center">
            <p className="text-sm text-ink-muted">No rep data available for this period.</p>
          </div>
        ) : (
            ...
            {repStats.map((rep, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 sm:gap-4">
                  <span className="w-6 text-xs text-ink-faint font-mono text-right">#{i + 1}</span>
                  <Avatar name={rep.name} size="sm" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium text-ink truncate">{rep.name}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-ink-muted">{rep.calls} calls</span>
        ...
        <div className="flex items-end justify-between gap-1 sm:gap-2 h-32 sm:h-40">
          {generateVolumeBars(filteredCalls).map((bar, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <div className="flex-1 w-full rounded-t-md bg-gradient-to-t from-accent-600/30 to-accent-500/50 transition-all duration-500 relative group" style={{ height: `${bar.height}%` }}>
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] text-accent-fg whitespace-nowrap">
                  {bar.count}
                </div>
              </div>
              <span className="text-[8px] sm:text-[10px] text-ink-faint">{bar.label}</span>
            </div>
          ))}
        </div>
      </ChartCard>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="h-8 w-52 rounded-lg bg-panel-raised animate-pulse" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="rounded-xl border border-edge bg-panel p-5 space-y-3">
            <div className="h-3 w-20 rounded bg-panel-raised animate-pulse" />
            <div className="h-8 w-16 rounded bg-panel-raised animate-pulse" />
        ...
        {[1, 2, 3].map((i) => (
          <div key={i} className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="h-3 w-24 rounded bg-panel-raised animate-pulse" />
              <div className="h-3 w-8 rounded bg-panel-raised animate-pulse" />
            </div>
            <div className="h-3 w-full rounded-full bg-panel-raised animate-pulse" />
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-edge bg-panel p-5 space-y-4">
        ...
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-4">
            <div className="h-4 w-6 rounded bg-panel-raised animate-pulse" />
