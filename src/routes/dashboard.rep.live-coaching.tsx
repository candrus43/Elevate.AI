    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-ink-faint dark:text-ink-muted">Real-time AI coaching during calls</p>
        </div>
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <span className="text-sm text-gray-600 dark:text-ink-muted">AI Coach</span>
            <button
              onClick={() => setCoachEnabled(!coachEnabled)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${coachEnabled ? "bg-accent-600" : "bg-gray-300 dark:bg-gray-600"}`}
              role="switch"
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-all ${
              isActive
                ? "bg-red-600 text-ink hover:bg-red-500"
                : "bg-green-600 text-ink hover:bg-green-500"
            }`}
          >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          {!isActive ? (
            <div className="rounded-xl border border-gray-200 bg-white px-6 py-12 text-center dark:border-gray-700 dark:bg-gray-900">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-ink mb-2">Ready for Your Next Call?</h3>
              <p className="text-sm text-ink-faint dark:text-ink-muted max-w-md mx-auto mb-6">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-green-600 px-6 py-3 text-sm font-medium text-ink transition-colors hover:bg-green-500"
              >
            ...
            <>
              ...
              <div className="rounded-xl border border-green-200 bg-white p-5 dark:border-green-800 dark:bg-gray-900">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-ink-faint">12:34 elapsed</span>
                  </div>
              ...
              <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
                <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-ink">AI Coach</h3>
                  {coachEnabled && (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.type === "praise" ? "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300" : "bg-accent-100 text-accent-700 dark:bg-accent-900/50 dark:text-accent-300"}`}>
                                {s.type === "praise" ? "Praise" : "Suggestion"}
                              </span>
                              {s.shown && <span className="text-xs text-ink-muted">Dismissed</span>}
                            </div>
                            <p className="text-sm text-gray-900 dark:text-ink">{s.text}</p>
                          </div>
                          {!s.shown && (
                            <button className="text-xs font-medium text-accent-600 hover:text-accent-500 dark:text-accent-400 whitespace-nowrap">Got it</button>
                          )}
                        </div>
                      </div>
                    ))
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-ink mb-4">Your Coaching Stats</h3>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <span className="text-sm text-ink-faint">{stat.label}</span>
                  <div className="text-right">
                    <span className="text-sm font-medium text-gray-900 dark:text-ink">{stat.value}</span>
                    {stat.change && (
          ...
          {/* Tips */}
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-ink mb-3">Quick Tips</h3>
            <div className="space-y-3">
              <div className="rounded-lg bg-accent-50 p-3 dark:bg-accent-900/20">
                <p className="text-xs text-accent-700 dark:text-accent-300">💡 Speak clearly and at a moderate pace for better AI analysis</p>
              </div>
