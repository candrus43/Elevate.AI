...
function AICertifications() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-ink">AI Certifications</h1>
        <p className="text-sm text-ink-faint">Earn certifications to validate your sales skills</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          { name: "Objection Handler", desc: "Handle objections like a pro", difficulty: "Advanced", progress: 45, icon: "🛡️", color: "from-accent-500 to-pink-500" },
          ...
          <div key={i} className="group rounded-xl border border-gray-200 bg-white p-5 transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-900">
            <div className="flex items-start justify-between mb-4">
              <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${cert.color} text-xl text-ink`}>
                {cert.icon}
              </div>
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                ...
                cert.difficulty === "Advanced" ? "bg-accent-100 text-accent-700 dark:bg-accent-900/50 dark:text-accent-300" :
                "bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300"
              }`}>{cert.difficulty}</span>
            </div>
            <h3 className="font-semibold text-gray-900 dark:text-ink mb-1">{cert.name}</h3>
            <p className="text-sm text-ink-faint mb-4">{cert.desc}</p>
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-ink-faint">Progress</span>
              <span className={`font-medium ${cert.progress === 100 ? "text-green-600" : "text-gray-900 dark:text-ink"}`}>
            ...
            {cert.progress < 100 && (
              <button className="mt-4 w-full rounded-xl bg-accent-600 py-2 text-sm font-medium text-ink transition-colors hover:bg-accent-500">Continue</button>
            )}
            {cert.progress === 100 && (
