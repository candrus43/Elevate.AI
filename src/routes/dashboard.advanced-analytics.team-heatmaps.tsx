...
function TeamHeatmaps() {
  ...
  const getColor = (value: number, metric: string) => {
    if (metric === "Improvement") {
      if (value >= 15) return "bg-emerald-500 text-ink";
      if (value >= 10) return "bg-emerald-300 text-emerald-900";
      if (value >= 7) return "bg-amber-300 text-amber-900";
      return "bg-red-400 text-ink";
    }
    if (value >= 85) return "bg-emerald-500 text-ink";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1"><h1 className="text-2xl font-bold text-ink">Team Heatmaps</h1><p className="text-sm text-ink-muted">Visualize team performance across metrics</p></div>
        <select value={department} onChange={(e) => setDepartment(e.target.value)} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-focus/40">
      ...
      <div className="flex items-center gap-4 rounded-xl border border-gray-200 bg-white px-6 py-3 dark:border-gray-700 dark:bg-gray-900">
        <span className="text-xs font-medium text-ink-muted">Legend:</span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-emerald-500" /><span className="text-xs text-ink-faint">Strong</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-emerald-300" /><span className="text-xs text-ink-faint">Good</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-amber-300" /><span className="text-xs text-ink-faint">Needs Work</span></span>
        <span className="flex items-center gap-1.5"><span className="h-3 w-6 rounded bg-red-400" /><span className="text-xs text-ink-faint">Weak</span></span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-edge">
              <th className="px-6 py-3 text-left text-xs font-semibold text-ink-faint uppercase">Team</th>
              {metrics.map(m => <th key={m} className="px-4 py-3 text-center text-xs font-semibold text-ink-faint uppercase">{m}</th>)}
            </tr>
          </thead>
          <tbody className="divide-y divide-edge">
            {teams.map((team, i) => (
              <tr key={i} className="transition-colors hover:bg-white/[0.02]">
                <td className="px-6 py-3 text-sm font-medium text-ink">{team}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setSelectedCell(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-900" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-ink">Metric Detail</h3>
              <button onClick={() => setSelectedCell(null)} className="text-2xl text-ink-muted hover:text-gray-600">&times;</button>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between"><span className="text-sm text-ink-faint">Team</span><span className="text-sm font-medium text-ink">{selectedCell.team}</span></div>
              <div className="flex items-center justify-between"><span className="text-sm text-ink-faint">Metric</span><span className="text-sm font-medium text-ink">{selectedCell.metric}</span></div>
              <div className="flex items-center justify-between"><span className="text-sm text-ink-faint">Value</span><span className={`text-lg font-bold ${selectedCell.value >= 75 ? "text-green-600" : selectedCell.value >= 65 ? "text-amber-600" : "text-red-600"}`}>{selectedCell.metric === "Improvement" ? `+${selectedCell.value}` : `${selectedCell.value}`}</span></div>
