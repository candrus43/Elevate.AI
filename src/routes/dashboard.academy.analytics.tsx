...
function LearningAnalytics() {
  ...
  const overview = [
    ...
    { label: "Avg Quiz Score", value: "84%", change: "+5%", color: "from-accent-500 to-accent-600" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-ink">Learning Analytics</h1><p className="text-sm text-ink-muted">Insights into learning engagement and outcomes</p></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {overview.map((o, i) => (
          <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
            <p className="text-sm text-ink-muted">{o.label}</p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl glass-subtle transition-all duration-300">
          <div className="border-b border-gray-200 px-5 py-3 dark:border-gray-700">
            <h3 className="text-sm font-semibold text-ink">Most Popular Content</h3>
          </div>
          <div className="divide-y divide-edge">
            {popular.map((item, i) => (
              <div key={i} className="flex items-center justify-between px-5 py-3.5 transition-colors hover:bg-white/[0.02]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
                      item.type === "Course" ? "bg-accent-100 text-accent-700 dark:bg-accent-900/50 dark:text-accent-300" :
                ...
                <span className="text-sm font-semibold text-ink">{item.completions}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-sm font-semibold text-ink mb-3">Skill Gap Analysis</h3>
              ...
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-ink-muted">{skill.score}%</span>
                    <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
      ...
      {/* Team Comparison */}
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-sm font-semibold text-ink mb-4">Team Comparison</h3>
        <div className="space-y-3">
          {teams.map((t, i) => (
            <div key={i} className="flex items-center gap-4">
              <span className="w-36 text-xs font-medium text-ink">{t.name}</span>
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-1">
                  <div className="flex-1 h-2 rounded-full bg-white/[0.06]">
                    <div className="h-2 rounded-full bg-accent-500" style={{ width: `${t.completion}%` }} />
                  </div>
                  <span className="text-[10px] text-ink-faint w-16 text-right">Completion: {t.completion}%</span>
