...
export const Route = createFileRoute("/executive/coaching/team-improvement")({
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-ink">Team Improvement Trends</h1>
          <p className="text-sm text-ink-muted">Track coaching impact and skill development over time</p>
        </div>
        <select value={selectedTeam} onChange={(e) => setSelectedTeam(e.target.value)} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-focus/40">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Score Improvement Over Time</h3>
          <div className="h-48">
          ...
          <div className="flex items-center gap-4 text-xs text-ink-faint mt-2">
            ...
            <span className="flex items-center gap-1"><span className="h-2 w-4 rounded bg-accent-400" /> Company Average</span>
          </div>
        </div>

        {/* Coaching Completion Rates */}
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          <h3 className="text-lg font-semibold text-ink mb-4">Coaching Completion Rates</h3>
          <div className="space-y-3">
            {completion.map((c, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="w-8 text-xs text-ink-faint">{c.month}</span>
                <div className="flex-1 h-3 rounded-full bg-white/[0.06]">
                  <div className="h-3 rounded-full bg-gradient-to-r from-accent-500 to-accent-500" style={{ width: `${c.rate}%` }} />
                </div>
                <span className="w-10 text-right text-xs font-semibold text-ink">{c.rate}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-2xl glass-subtle transition-all duration-300">
        <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-ink">Per-Skill Improvement Breakdown</h3>
        </div>
              ...
              <div key={i} className="px-6 py-4 transition-colors hover:bg-white/[0.02]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-ink-muted">Pre: {s.pre}%</span>
                    <span className="text-xs font-bold text-green-600">+{improvement}%</span>
                    <span className="text-xs text-ink-muted">Post: {s.post}%</span>
                    <span className="text-xs text-accent-400">Avg: {s.avg}%</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-3 rounded-full bg-white/[0.06] relative overflow-hidden">
                    <div className="h-3 rounded-full bg-accent-300 dark:bg-accent-700" style={{ width: `${s.pre}%` }} />
                ...
                <div className="flex items-center gap-3 text-[10px] text-ink-muted mt-1">
                  <span className="flex items-center gap-0.5"><span className="h-1.5 w-2 rounded bg-accent-300" /> Pre</span>
