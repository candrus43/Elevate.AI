  ...
  return (
    <div
      className={`rounded-2xl transition-all duration-300 ${
        hover ? "hover:border-accent-500/25 hover:translate-y-[-1px]" : ""
      } ${glow ? "shadow-lg shadow-accent-500/10" : ""} ${paddingClasses[padding]} ${className}`}
      style={{
        background: "linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(51,143,255,0.04) 50%, rgba(255,255,255,0.02) 100%)",
...
export function GlassCardBody({ children, className = "", divide = false }: { children: ReactNode; className?: string; divide?: boolean }) {
  return (
    <div className={`${divide ? "divide-y divide-edge" : ""} ${className}`}>
      {children}
    </div>
  );
}
...
export function GlassBadge({
  ...
  const colorClasses = {
    default: "text-ink-muted bg-panel-raised",
    purple: "text-accent-300 bg-accent-500/10",
...
export function GlassButton({
  ...
  const variants = {
    primary: "bg-gradient-to-r from-accent-600 to-accent-500 text-white shadow-lg shadow-accent-500/25 hover:shadow-xl hover:shadow-accent-500/30 hover:from-accent-500 hover:to-accent-400",
...
export function GlassInput({
  ...
  return (
    <input
      {...props}
      className="w-full rounded-xl px-3 sm:px-4 py-2.5 text-sm text-ink placeholder-ink-faint focus:outline-none focus:ring-1 focus:ring-focus/40 transition-all"
      style={{
...
export function GlassSelect({
  ...
  return (
    <select
      {...props}
      className="rounded-xl px-3 py-2 text-sm text-ink focus:outline-none focus:ring-1 focus:ring-focus/40 transition-all appearance-none"
      style={{
        background: "rgba(255, 255, 255, 0.05)",
        border: "1px solid rgba(255, 255, 255, 0.08)",
      }}
